...
var toTypeString = function (value) { return Object.prototype.toString.call(value); };
...
var createVNode = function (type, props, children) {
    ...
    if (Array.isArray(children)) {
        vnode.shapeFlag |= shared_.ShapeFlags.ARRAY_CHILDREN;
    }
    else if (typeof children === "string") {
        vnode.shapeFlag |= shared_.ShapeFlags.TEXT_CHILDREN;
    }
...
function normalizeChildren(vnode, children) {
    if (typeof children === "object") {
        if (vnode.shapeFlag & shared_.ShapeFlags.ELEMENT) ;
        else {
            vnode.shapeFlag |= shared_.ShapeFlags.SLOTS_CHILDREN;
        }
    }
}
...
function getShapeFlag(type) {
    return typeof type === "string"
        ? shared_.ShapeFlags.ELEMENT
        : shared_.ShapeFlags.STATEFUL_COMPONENT;
}
...
function initSlots(instance, children) {
    ...
    if (vnode.shapeFlag & shared_.ShapeFlags.SLOTS_CHILDREN) {
...
function emit(instance, event) {
    ...
    var handlerName = index.toHandlerKey(index.camelize(event));
...
function patch(n1, n2, container, parentComponent) {
    ...
    switch (type) {
        case Text:
            ...
            break;
        default:
            if (shapeFlag & shared_.ShapeFlags.ELEMENT) {
                console.log("处理 element");
                processElement(n1, n2, container);
            }
            else if (shapeFlag & shared_.ShapeFlags.STATEFUL_COMPONENT) {
...
function patchChildren(n1, n2, container) {
    ...
    if (shapeFlag & shared_.ShapeFlags.TEXT_CHILDREN) {
        if (c2 !== c1) {
        ...
        if (prevShapeFlag & shared_.ShapeFlags.ARRAY_CHILDREN) {
            if (shapeFlag & shared_.ShapeFlags.ARRAY_CHILDREN) {
...
function mountElement(vnode, container) {
    ...
    else if (shapeFlag & shared_.ShapeFlags.ARRAY_CHILDREN) {
