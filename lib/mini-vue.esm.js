import { ShapeFlags as ShapeFlags$1 } from '\'../shared\'';
...
var createVNode = function (type, props, children) {
    ...
    if (Array.isArray(children)) {
        vnode.shapeFlag |= ShapeFlags$1.ARRAY_CHILDREN;
    }
    else if (typeof children === "string") {
        vnode.shapeFlag |= ShapeFlags$1.TEXT_CHILDREN;
    }
...
function normalizeChildren(vnode, children) {
    if (typeof children === "object") {
        if (vnode.shapeFlag & ShapeFlags$1.ELEMENT) ;
        else {
            vnode.shapeFlag |= ShapeFlags$1.SLOTS_CHILDREN;
        }
    }
}
...
function getShapeFlag(type) {
    return typeof type === "string"
        ? ShapeFlags$1.ELEMENT
        : ShapeFlags$1.STATEFUL_COMPONENT;
}
...
function initSlots(instance, children) {
    ...
    if (vnode.shapeFlag & ShapeFlags$1.SLOTS_CHILDREN) {
...
function patch(n1, n2, container, parentComponent) {
    ...
    switch (type) {
        case Text:
            ...
            break;
        default:
            if (shapeFlag & ShapeFlags$1.ELEMENT) {
                console.log("处理 element");
                processElement(n1, n2, container);
            }
            else if (shapeFlag & ShapeFlags$1.STATEFUL_COMPONENT) {
...
function patchChildren(n1, n2, container) {
    ...
    if (shapeFlag & ShapeFlags$1.TEXT_CHILDREN) {
        if (c2 !== c1) {
        ...
        if (prevShapeFlag & ShapeFlags$1.ARRAY_CHILDREN) {
            if (shapeFlag & ShapeFlags$1.ARRAY_CHILDREN) {
...
function mountElement(vnode, container) {
    ...
    else if (shapeFlag & ShapeFlags$1.ARRAY_CHILDREN) {
